...
`exit_plan_mode` requires a calling agent in active plan mode and a non-empty markdown plan beginning with a heading. The first user-questions request carries that exact plan as detail and offers `Approve` or `Keep planning` plus free-text feedback. Only one `Approve` selection with no custom text consents; every other answer stays in plan mode and returns corrective feedback to the model. Approval opens a second generic question with one option per configured worker count. A valid option or numeric custom answer returns `{ approved: true, execution_agents }`; the rendered result directs the lead to create that many workers, give each a distinct task, preserve dependency and overlapping-write order, wait for every worker, integrate, and validate. The exit becomes a silent pending selection only after both questions succeed, leaving plan guidance active for the rest of the current tool batch and removing it before the next request. Dismissal, invalid selection, or service disposal during either question fails closed.

The tool renders the submitted plan as a generic card titled by its first heading. An absent or failed user-questions provider, a failed question, or plugin disposal while either question is pending fails closed and leaves manual `/plan off` as the human escape path.
