...
`plan/` 组提供计划模式：激活期间，agent 先探索和设计再执行，遵循部署写入的指令，并在执行前提交完成的计划供你批准。你可以用 `/plan` 命令进入和离开计划模式，批准计划，或让 agent 回去继续规划。计划模式是引导而非限制：每个工具仍然可用，沙箱模式与审批提示等限制需另行配置。该组包含两个包：`plan-mode`，以及把两个阶段路由到不同模型、并折叠已通过计划背后探索过程的 `plan-model-switch`。
...
一个包提供计划模式功能本身，另一个在其阶段之间路由模型；完整约定以子系统参考为准。

| 包 | 职责 | ctx 键 |
|---|---|---|
| [`plan-mode/`](plan-mode/README.zh.md) | 提供计划模式：`/plan` 进入和离开，部署写入的指令在规划期间引导 agent，`exit_plan_mode` 把完成的计划呈交你评审 | `ctx.planMode` |
| [`plan-model-switch/`](plan-model-switch/README.zh.md) | 把规划与执行路由到不同模型，并把已通过计划背后的探索折叠成一段摘要 | 不注册服务 |
