...
随附 Web 的 `standard`、`ptc` 与 `cordis` preset 包含[显式文件交付](../../client/ui-deliverables/README.zh.md#explicit-deliveries)。`minimal` preset 保留固定的双工具训练配置。 `plan-execute` preset 是 `standard` 加上阶段模型路由（[`dsh-plan-model-switch`](../../plan/plan-model-switch/README.zh.md)）：两条路由都配置后，它规划用一个模型、执行用另一个模型；未配置时与 `standard` 完全一致。
