...
The shipped Web `standard`, `ptc`, and `cordis` presets include [explicit file delivery](../../client/ui-deliverables/README.md#explicit-deliveries). The `minimal` preset keeps its fixed two-tool training configuration. The `plan-execute` preset is `standard` plus phase model routing ([`dsh-plan-model-switch`](../../plan/plan-model-switch/README.md)): it plans on one model and executes on another once both routes are configured, and behaves exactly like `standard` until they are.
